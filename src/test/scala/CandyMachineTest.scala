...
class CandyMachineTest extends UnitSpec {
  ...
  "combining inserting coin and knob turn" should "work" in {
    val x = CandyMachine.simulate2(MyList(Coin, Turn))
    val r = x.run(CandyMachine(true, 1, 0))
    assert(r._2.coins == 1)
    assert(r._2.candies == 0)
    assert(r._2.locked == true)
  ...
  "combining many actions" should "work" in {
    val x = CandyMachine.simulate2(MyList(Coin, Turn, Coin, Turn, Turn))
    val r = x.run(CandyMachine(false, 10, 10))
    assert(r._2.coins == 11)
    assert(r._2.candies == 8)
    assert(r._2.locked == true)
  }
